...
# FIXME: defaults if no arguments
...
# pointing to zyn install
import os
sys.path.insert(0, os.path.join(
    os.path.dirname(os.path.realpath(__file__)), ZYNTHIAN_DIR + "/zynthian-emuface/"))
...
class MainWindow(QtGui.QMainWindow):
    ...
    def __init__(self):
        ...
        if len(sys.argv)>5:
            self.zynthian_pid=int(sys.argv[5])
        else:
            self.start_zynthian()
